...
local HitSoundLibrary = 
{
    SoundAssetId = 'SoundNotLoaded',
    Initialize = function(self)
        if not isfile('Inferno/HitSound.wav') then
            local success, errorMessage = pcall(function()
                writefile(
                    'Inferno/HitSound.wav',
                    game:HttpGet(
                        'https://inferno.rocks/ScriptHub/SkeetHitsound.wav'
                    )
                )
            end)
        ...
        if isfile('Inferno/HitSound.wav') then
            self.SoundAssetId = getcustomasset('Inferno/HitSound.wav')
        end
    end,
    ...
    PlayHitSound = function(self, SoundParent, Volume)
        ...
        sound.SoundId = self.SoundAssetId
...
local Dependencies =
{
    VerifyAssets = function(self)
        local assets = loadstring(game:HttpGet("https://inferno.rocks/ScriptHub/Available.lua"))()

        for i,v in pairs(assets) do
            if (not isfile("Inferno/Icons/" .. v .. ".png")) then
                local success, errorMessage = pcall(function()
                    writefile("Inferno/Icons/" .. v .. ".png", game:HttpGet("https://github.com/quadshoota/Inferno/blob/main/Icons/" .. v .. ".png?raw=true"))
                    writefile("Inferno/Icons/" .. v .. ".png", game:HttpGet("https://inferno.rocks/ScriptHub/Icons/" .. v .. ".png?raw=true"))
                end)

                if (not success) then
                    warn("Failed to download asset: " .. v .. " - " .. errorMessage)
                else
                    Storage.Icons[v] = "rbxasset://Inferno/Icons/" .. v
                    warn("Downloaded asset: " .. v)
                end
            end

            if (isfile("Inferno/Icons/" .. v .. ".png")) then
                Storage.Icons[v] = "rbxasset://Inferno/Icons/" .. v
            end
        end
    end,
    ...
    CustomAsset = function(self, assetName) 
        ...
        local path = "Inferno/Icons/" .. assetName .. ".png"
...
local VisualEffects = 
{
    ...
    CreateCustomDamageMarker = function(self, targetPosition, damageText, color, darkerColor)
        ...
        billboardGui.Frame.DamageShadow.Text = damageText or 'Inferno.Rocks Error'
        billboardGui.Frame.DamageShadow.Damage.Text = damageText or 'Inferno.Rocks Error'
...
local Subsections = 
{
    ...
    -- Items/Cards
    Cards = Sections.Items:Subsection({
        Name = "Cards",
        Side = "Right",
    }),
...
local Config =
{
    ...
    Farm =
    {
        AutoCombat = false,
        AutoMove = false,
        AutoMovePriority = {"Health", "Rarity"},
        SelectedAutoMovePriority = "Health",
        AutoMoveOptions = {"Equip Best Plants", "Use Amount", "DPS Based Amount"},
        SelectedAutoMoveOptions = {},
        UseAmount = 5,
        OriginalUse = 5,

        Priority = {"Distance", "Rarity", "Health", "Mutation", "Low"},
        SelectedPriority = "Distance",
    },
...
local Features =
{
    Farm =
    {
        ...
        AttackEntity = function(self)
            ...
            Utils:EquipTool(true)
            combatevent:FireServer({
                target.Name,
            })

            if (Config.UserInterface.PlayHitsound == false and Config.UserInterface.ShowEffects == false) then
                return
            end

            if (Config.UserInterface.PlayHitsound) then
                HitSoundLibrary:PlayHitSound(
        ...
        PickupPlants = function(self, row)
            ...
            for i,v in pairs(plot.Plants:GetChildren()) do
                ...
                if (Utils:Contains(Config.Farm.SelectedAutoMoveOptions, "Use Amount") and count < Config.Farm.UseAmount) then
                    count = count + 1
                elseif (Utils:Contains(Config.Farm.SelectedAutoMoveOptions, "Use Amount") and count >= Config.Farm.UseAmount) then
                    break
                end
        ...
        PlacePlants = function(self, row, dpsneeded)
            ...
            local tomove = 35
            if (Utils:Contains(Config.Farm.SelectedAutoMoveOptions, "Use Amount")) then
            ...
            if (Utils:Contains(Config.Farm.SelectedAutoMoveOptions, "Equip Best Plants")) then
                for i, v in pairs(lcl.Backpack:GetChildren()) do
        ...
        AutoMovePlants = function(self)
            ...
            for i,v in pairs(brainpath:GetChildren()) do
                ...
                if (Config.Farm.SelectedAutoMovePriority == "Health" and maxhealth > highesthealth) then
...
Subsections.Favorites:Separator()

Subsections.Favorites:Dropdown({
    Name = "Options",
...
Subsections.Favorites:Paragraph({
    Title = "» Information",
    Description = {
        {
            Text = "Options » Select criteria to favorite.",
        },
        {
            Text = "Rarities » Select rarities to favorite.",
        },
        {
            Text = "Weight » Favorite items above weight.",
        },
        {
            Text = "Value » Favorite items above value.",
        },
    },
    Position = "Center",
})
...
Subsections.Projectiles:Textbox({
    Name = "Health Threshold",
    Flag = "ProjectileHealth",
    PlaceholderText = "Select health..",
